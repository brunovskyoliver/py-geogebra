...
def motion(root):
    def handle_mouse(e):
        ...
        elif state.selected_tool == "segment" and 0 < len(state.points_for_obj) < 3:
            state.points_for_obj[1].update(e)

        elif state.selected_tool == "ray" and 0 < len(state.points_for_obj) < 3:
            state.points_for_obj[1].update(e)
        ...
        elif state.selected_tool == "perpendicular_bisector" and 1 <= len(state.points_for_obj) <3:
            state.points_for_obj[1].update(e)

        elif state.selected_tool == "circle_center_point" and 0 < len(state.points_for_obj) < 3:
            state.points_for_obj[1].update(e)
